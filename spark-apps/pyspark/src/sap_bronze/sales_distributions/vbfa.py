...
from pyspark.sql.functions import (col, when, lit, to_date, 
                                   date_format, trim, length, current_timestamp, to_timestamp)
...
def load_config(env_path: Path | None = None) -> dict:
    ...
    cfg = {
        ...
        "LANDING_URI": os.getenv("LANDING_URI", "s3a://landing/SAP/vbfa.csv"),
        "BRONZE_URI": os.getenv("BRONZE_URI", "s3a://bronze/sap/vbfa/"),
