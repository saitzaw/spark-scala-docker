# vbrp.py : version 1.02 Date 2025-10-12
###################################################################################
...
def transform(df: DataFrame) -> DataFrame:
    ...
    df = df.withColumn(
        "prsdt",
            when(
                    (col("prsdt").isNull()) | (length(trim(col("prsdt")))==0),
                    lit('9999-12-31')
                ).otherwise(
                    date_format(to_date(col("prsdt"),"dd/MM/yyyy"), "yyyy-MM-dd")
                )
        ).withColumn(
        "fbuda",
            when(
                    (col("fbuda").isNull()) | (length(trim(col("fbuda")))==0), 
                    lit('9999-12-31')
                ).otherwise(
        ...
        "kursk_dat",
            when(
                    (col("kursk_dat").isNull()) | (length(trim(col("kursk_dat")))==0),
                    lit('9999-12-31')
                ).otherwise(
