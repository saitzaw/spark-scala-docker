# vbap.py : version 1.00 Date 2025-10-04
###################################################################################
...
def load_config(env_path: Path | None = None) -> dict:
    ...
    cfg = {
        ...
        "LANDING_URI": os.getenv("LANDING_URI", "s3a://landing/SAP/vbap.csv"),
        "BRONZE_URI": os.getenv("BRONZE_URI", "s3a://bronze/sap/vbap/"),
...
def transform(df: DataFrame) -> DataFrame:
    ...
    df = df.withColumn(
        ...
        when((col("session_creation_date").isNull()) | trim(col("session_creation_date)" == ""))), lit('9999-12-31')).otherwise(
