...
def load_config(env_path: Path | None = None) -> dict:
    ...
    cfg = {
        ...
        "BRONZE_VBAK_URI": os.getenv("BRONZE_VBAK_URI","s3a://bronze/sap/vbak/"), 
    ...
    cfg["SRC_VBAK"] = cfg["BRONZE_VBAK_URI"] or f'{cfg["BRONZE_ROOT"].rstrip("/")}/vbak/'
    cfg["TGT_PATH"] = cfg["SILVER_SO_HDR_URI"] or f'{cfg["SILVER_ROOT"].rstrip("/")}/sales_order_header_scd2/'
    cfg["TGT_TABLE"] = f'{cfg["HIVE_DB"]}.sales_order_header_scd2'
    return cfg
...
def build_spark(cfg: dict) -> SparkSession:
    builder = (
        ...
        .appName("SILVER_SALES_ORDER_HEADER_SCD2")
...
# --- Helpers ---
# =========================
BK = ["client", "sales_order"]  # business key
TRACKED_ATTRS = [
    "doc_type", "sales_org", "dist_channel", "division",
    "order_date", "created_on", "changed_on",
    "sold_to", "currency", "net_value"
]
...
def make_hash(df, cols):
    return df.withColumn("_attr_hash", F.md5(F.concat_ws("||", *[F.col(c).cast("string") for c in cols])))
...
def build_snapshot(spark: SparkSession, cfg: dict):
    vbak = spark.read.load(cfg["SRC_VBAK"])  # parquet or delta; Spark infers

    snap = (
        vbak
        .withColumn("client", F.col("mandt"))
        .withColumn("sales_order", null_if_blank("vbeln"))
        .withColumn("doc_type", F.col("auart"))
        .withColumn("sales_org", F.col("vkorg"))
        .withColumn("dist_channel", F.col("vtweg"))
        .withColumn("division", F.col("spart"))
        .withColumn("order_date", parse_sap_date("audat"))
        .withColumn("created_on", parse_sap_date("erdat"))
        .withColumn("changed_on", parse_sap_date("aedat"))
        .withColumn("sold_to", null_if_blank("kunnr"))
        .withColumn("currency", null_if_blank("waerk"))
        .withColumn("net_value", F.col("netwr").cast("double"))
        # keep tech columns if present
    ...
    snap = snap.where(F.col("sales_order").isNotNull())

    # Deduplicate by latest change/ingest
    w = Window.partitionBy(*BK).orderBy(F.coalesce(F.col("_change_ts"), F.col("_ingest_ts")).desc_nulls_last())
    snap = (snap.withColumn("_rn", F.row_number().over(w))
                 .where(F.col("_rn") == 1)
                 .drop("_rn"))

    snap = make_hash(
        snap.select(*(BK + TRACKED_ATTRS + ["_ingest_ts","_change_ts"])),
        TRACKED_ATTRS
    )
    return snap
...
def initial_load(spark: SparkSession, cfg: dict, snap):
    ...
    out = (snap
           ...
           .withColumn("sk_sales_order", F.md5(F.concat_ws("||", *[F.col(k).cast("string") for k in BK])))
          )
    (out.write
        .format("delta")
        .mode("overwrite")
...
def incremental_load(spark: SparkSession, cfg: dict, snap):
    ...
    # 1) Expire changed active rows (hash mismatch)
    ...
    (tgt.alias("t")
        .merge(to_expire.alias("e"), " AND ".join([f"t.{k} = e.{k}" for k in BK]) + " AND t.is_current = true")
        .whenMatchedUpdate(set={"end_ts": F.lit(load_time), "is_current": F.lit(False)})
        .execute()
    )

    # 2) Insert new versions (brand-new keys + changed keys)
    ...
    staged = (
        ...
        .select(
            ...
            *TRACKED_ATTRS,
            ...
            "start_ts","end_ts","is_current","version","sk_sales_order"
        )
    )
